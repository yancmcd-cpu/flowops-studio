...
function ExpandingSection({ section, idx }) {
  ...
  return (
    <section ref={ref} className="expanding-section">
      ...
      <div className="expanding-visual-container">
        <motion.div 
          style={{ width, borderRadius }} 
          className="expanding-player"
        >
...
export default function PrototypeC({ onBack }) {
  return (
    <div className="flowops-app proto-c">
      <nav className="proto-nav">
        <button onClick={onBack} className="back-btn"><ArrowLeft size={16} /> Back to Prototypes</button>
        <div className="badge">Prototype C: Hybrid Expanding Grid</div>
      </nav>
      ...
      <div className="expanding-container">
        {textContent.map((section, idx) => (
          <ExpandingSection key={idx} section={section} idx={idx} />
        ))}
      </div>
